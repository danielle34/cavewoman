"""Step 3a (Haiku variant), Claude Haiku 4.5 inference on LLMLingua-compressed prompts (single level).

Outputs:
    results/llmlingua/inference/haiku/<dataset>/<dataset>_LLMLingua.jsonl
"""
...
OUT_BASE = REPO / "results" / "llmlingua" / "inference" / "haiku-4.5"
...
def call_haiku(client, prompt: str, max_retries: int = 4):
    last_err = None
...
def run_one(dataset: str, client, shard: int, num_shards: int,
    ...
    with out_path.open("a") as f:
        for rec in todo:
            ...
            out_text, in_tok, out_tok, api_model, latency, err = call_haiku(client, prompt)
