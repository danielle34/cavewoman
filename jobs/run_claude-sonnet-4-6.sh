#!/bin/bash
# Run Claude Sonnet 4.6 on all 5 datasets × 2 conditions sequentially.
...
# ---- load .env and verify Anthropic key ----
if [ ! -f "$REPO_ROOT/.env" ]; then
    echo "[ERROR] $REPO_ROOT/.env not found. Create it with ANTHROPIC_API_KEY=sk-ant-..." >&2
    exit 1
fi
set -a
...
if [ -z "${ANTHROPIC_API_KEY:-}" ] || [[ "$ANTHROPIC_API_KEY" == your_* ]]; then
    echo "[ERROR] ANTHROPIC_API_KEY not set or still placeholder in $REPO_ROOT/.env" >&2
    echo "        Edit the repository .env and replace 'your_anthropic_key_here' with a real sk-ant-... key." >&2
    exit 1
fi
echo "[claude-sonnet-4-6] ANTHROPIC_API_KEY loaded (length: ${#ANTHROPIC_API_KEY})"
...
# CONDITIONS defaults to "output input" but can be overridden via env var, e.g.
#   CAVEWOMAN_CONDITIONS=output  bash run_sonnet_all.sh   # output-only sweep
#   CAVEWOMAN_CONDITIONS=input   bash run_sonnet_all.sh   # input-only sweep
# Useful for parallel runs in two tmux sessions (~2x speedup).
read -ra CONDITIONS <<< "${CAVEWOMAN_CONDITIONS:-output input}"
LEVELS=(L0 L1 L2 L3 L4)

echo "===================================================================="
echo "[claude-sonnet-4-6] START $(date -Iseconds) on $(hostname)"
...
for DATASET in "${DATASETS[@]}"; do
    for CONDITION in "${CONDITIONS[@]}"; do
        ...
        LOG="$LOGS_DIR/sonnet_${DATASET}_${CONDITION}.log"

        echo
        echo "----------------------------------------------------------------"
        echo "[claude-sonnet-4-6] $(date -Iseconds)  ${DATASET} / ${CONDITION}"
        echo "----------------------------------------------------------------"
...
echo
echo "===================================================================="
echo "[claude-sonnet-4-6] END $(date -Iseconds)"
echo "===================================================================="
